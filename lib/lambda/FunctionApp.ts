...
exports.handler = async (event:any) => {
  ...
  return `
      <html>
        <body>
          <h2>Welcome, ${username} <input type='button' value='logout' onclick="document.location.href = '${rootUrl}/logout';" /></h2>
          <p>Here is what shibboleth "said" about you:</p>
          <pre>${preContent}</pre>
        </body>
      </html>
    `;
